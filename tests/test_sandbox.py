...
def test_egress_requires_dedicated_network():
    policy = TaskSandboxPolicy(image="worker:test", network="egress", egress_network="bridge")
...
def test_command_is_resource_constrained_and_isolated(tmp_path):
    policy = TaskSandboxPolicy(image="worker:test", limits=TaskResourceLimits(cpus=1.5, memory="1g", pids=128, disk="4g"))
...
def test_egress_command_uses_policy_network(tmp_path):
    policy = TaskSandboxPolicy(image="worker:test", network="egress", egress_network="policy-egress")
