...
def test_calibrator_requires_minimum_samples_and_quality_floor():
    ...
    assert (
        calibrator.select(["cheap", "fallback"], "code", fallback="fallback")
        == "fallback"
    )
    calibrator.record(RouteObservation("cheap", "code", True, 100, quality=0.95))
    assert (
        calibrator.select(["cheap", "fallback"], "code", fallback="fallback") == "cheap"
    )
...
def test_calibrator_recency_changes_effective_weight():
    ...
    calibrator.record(
        RouteObservation("route", "code", True, 100, quality=1.0, recorded_at=now)
    )
